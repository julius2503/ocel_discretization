...
def tranform_ocel(ocel: OCEL, items):
    ...
    for event_id in ocel.events["ocel:eid"].to_list():
        ...
        objects = ocel.objects[
            ocel.objects["ocel:oid"].isin(relations["ocel:oid"])
        ]
        for item in items:
            item = item[0]
            if item["type"] == "EVENT":
                if item["qualifier"] in events["ocel:activity"].tolist():
                    if "interval" in item:
                        if (
                            ...
                            transaction.append(
                                f"{item['attribute']}_{item['type']}_{item['qualifier']}_{item['interval']['start']}-{item['interval']['end']}"
                            )
                    elif "value" in item:
                        if (
                            events[item["attribute"]].astype(str)
                            == item["value"]
                        ).any():
                            transaction.append(
                                f"{item['attribute']}_{item['type']}_{item['qualifier']}_{item['value']}"
                            )
